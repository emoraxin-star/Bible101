...
uint32_t CRC32C_SW(const uint8_t* data, size_t len)
{
    ...
    uint32_t crc = HASH_SEED_CRC32C;
    for (size_t i = 0; i < len; i++)
        crc = table[(crc ^ data[i]) & 0xFF] ^ (crc >> 8);
    return crc ^ HASH_SEED_CRC32C;
}
...
uint32_t CRC32C_HW(const uint8_t* data, size_t len)
{
    ...
    {
        for (; i < len; i++)
            crc = (uint32_t)_mm_crc32_u8(crc, data[i]);
    }
...
{
    uint32_t hash = HASH_SEED_JENKINS;
    while (*str)
    {
        hash += (uint8_t)*str++;
        hash += (hash << 10);
...
HMODULE ResolveModuleByHash(uint32_t targetHash, uint32_t (*hashFunc)(const char*))
{
    ...
    {
        do {
            char ansiName[MAX_MODULE_NAME32 + 16];
            size_t converted = 0;
            wcstombs_s(&converted, ansiName, sizeof(ansiName), me.szModule, _TRUNCATE);

            uint32_t h = hashFunc(ansiName);
            if (h == targetHash)
            {
                result = me.hModule;
                break;
            ...
            char ansiExe[MAX_PATH];
            converted = 0;
            wcstombs_s(&converted, ansiExe, sizeof(ansiExe), me.szExePath, _TRUNCATE);
            h = hashFunc(ansiExe);
            if (h == targetHash)
            {
                result = me.hModule;
...
void* ResolveExportByHash(HMODULE moduleBase, uint32_t functionHash)
{
    ...
    if (ed->Size == 0 || ed->VirtualAddress == 0)
        return NULL;

    IMAGE_EXPORT_DIRECTORY* exports = (IMAGE_EXPORT_DIRECTORY*)(base + ed->VirtualAddress);
    ...
    for (uint32_t i = 0; i < exports->NumberOfNames; i++)
    {
        ...
        if (h == functionHash)
        {
            uintptr_t funcAddr = (uintptr_t)(base + functions[ordinals[i]]);
            if (funcAddr >= (uintptr_t)base + ed->VirtualAddress &&
                funcAddr < (uintptr_t)base + ed->VirtualAddress + ed->Size)
            {
                funcAddr = *(uintptr_t*)funcAddr;
            }
...
uint32_t HashModuleName(const wchar_t* modulePath)
{
    ...
    const wchar_t* dot = wcsrchr(last, L'.');
    size_t nameLen = dot ? (size_t)(dot - last) : len;

    char ansi[260];
    size_t converted = 0;
    wcstombs_s(&converted, ansi, sizeof(ansi), last, min(nameLen, 259));
    ansi[min(nameLen, (size_t)259)] = 0;
